    ...
    editPassword: async (
      ...
      args: { email: string; currentPassword: string; newPassword: string }
        ...
        try {
            ...
            if (
                !await bcrypt.compare(args.currentPassword, (user[0] as UserType).password)
            )
        ...
        loginUser: async (_ : unknown, args: { email : string, password: string }) => {
            try {
                ...
                await query('INSERT INTO refresh_tokens VALUES (?, ?)', [refreshToken, (user[0] as UserType).id]).then((res)=>{
                    console.log(res);
                });
                return {
                    refreshToken,
      ...
      getUserData: async (_ : unknown, args: { refreshToken : string }) => {
        try{
            const decoded = jwt.verify(args.refreshToken, `${process.env.REFRESH_TOKEN_SECRET}`)
            const user = await query('SELECT * FROM users WHERE id = ? AND verified = 1', [decoded.id])
             const email = user[0].email;
             const username = user[0].username;

             let avatar = await query('SELECT * FROM users_avatars WHERE id_user = ?', [decoded.id])
             if(avatar[0]==undefined)
                 avatar = "https://res.cloudinary.com/dukhbkqnv/image/upload/v1618869557/default-avatar-1_jzug3b.jpg";
            else
                avatar = avatar[0].link;


            let travels = await query('SELECT image_url FROM travel_images INNER JOIN travels ON travel_images.travel_id = travels.id WHERE user_id = ?', [decoded.id])
      ...
      changeUserPhoto: async (_: unknown, args: {refreshToken: string, file:string})=>{
        try{
            let linkImage = "";
            let photo="";
            cloudinary.v2.config({
             ...
             cloudinary.v2.uploader.upload(args.file, {
                 upload_preset:"ml_default"
             }).then((res)=>{
                 const decoded = jwt.verify(args.refreshToken, `${process.env.REFRESH_TOKEN_SECRET}`)
                 linkImage = res.url;

                query('SELECT * FROM users_avatars WHERE id_user = ?', [decoded.id]).then((res)=>{
                     if(res==undefined) {
                         let queryInsertPhoto = query('INSERT INTO users_avatars (id_user, link) VALUES (?,?)', [decoded.id, linkImage])
                     }else{
                         let queryUpdatePhoto  = query('UPDATE users_avatars SET link=? WHERE id_user=?', [linkImage, decoded.id]).then((res)=>{
